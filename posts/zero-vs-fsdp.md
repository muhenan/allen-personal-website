...
date: "2026-03-28"
description: "从 16N GB 显存公式出发，逐级拆解 ZeRO-1/2/3 的切分逻辑，再对比 PyTorch 原生 FSDP 在工程实现上的核心差异。"
---
...
假设训练一个参数量为 $N$ B 的模型（例如 $N = 70$，即 70B），采用混合精度训练（Mixed Precision，FP16/BF16），模型状态（Model States）的显存占用如下：

- **FP16 权重（Parameters）**：$2N$ GB（70B 模型约 140 GB）
- **FP16 梯度（Gradients）**：$2N$ GB（70B 模型约 140 GB）
- **Adam 优化器状态（Optimizer States）**：为了防止精度丢失，优化器需在 FP32 下更新。这包含 FP32 的主权重（Master Weights, $4N$ GB）、一阶矩（Momentum, $4N$ GB）和二阶矩（Variance, $4N$ GB），共计 $12N$ GB（70B 模型约 840 GB）。

**总计**：仅维持模型状态的基础显存就需要 $16N$ GB。对于 70B 模型，这高达 **1.12 TB**，远超单张 80G A100/H100 的容量。

传统的 Data Parallelism (DP) 要求每张卡保留完整的 $16N$ GB 状态，产生极大冗余。ZeRO 和 FSDP 的核心思路就是打破 DP 的限制，将这 $16N$ GB 的显存开销切片并分摊到集群的多个 GPU 上。
...
**机制**：权重和梯度依然每卡保留全量（各 $2N$ GB），但将最占显存的 Adam 状态（$12N$ GB）切成若干份。卡 $i$ 只负责更新属于自己的那一块权重。
